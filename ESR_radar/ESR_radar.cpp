...
bool ESR_RADAR::init()                                                  //初始化参数
{
	ros::init(argc_,argv_,"esr_radar");                                     //创建节点
	ros::NodeHandle nh_private("~");
	ros::NodeHandle nh;
	nh_private.param<std::string>("in_port_name", in_port_name_, "/dev/ttyUSB1");        //串口转USB端口设置
	nh_private.param<std::string>("out_port_name", out_port_name_, "/dev/ttyUSB2");
	
	nh_private.param<bool>("is_pubBoundingBox",is_pubBoundingBox_,false);        //ROS对外不发布
	
	nh_private.param<bool>("is_sendMsgToEsr",is_sendMsgToEsr_,false);
	
	ROS_INFO("in_port_name:%s",in_port_name_.c_str());        //打印参数值 C++类，字符串
	...
	if(!in_can2serial->configure_port(in_port_name_.c_str()))       //设置失败
	{
...
void ESR_RADAR::run() 
{
	boost::thread parse_thread(boost::bind(&ESR_RADAR::handleCanMsg,this));      //创建解析线程
	
	if(is_pubBoundingBox_)    //如果发布，启动下面线程
		this->start_publishBoundingBoxArray_thread();    //可视化数据类型
	
	if(is_sendMsgToEsr_)    //发给毫米波
	{
		ros::NodeHandle nh;
...
void ESR_RADAR::start_publishBoundingBoxArray_thread()    //开辟新线程
{
...
void ESR_RADAR::handleCanMsg()
{
	...
	while(ros::ok())
	{
		...
		parse_msg(can_msg);    //解析函数
		
	}
}


void ESR_RADAR::parse_msg(CanMsg_t &can_msg)    //解析函数
{
...
*/	
	...
	if(can_msg.ID <= 0x53f && can_msg.ID >=0x500)   //检测障碍物
	{
		if(objects.objects.size() > 63)
		{
			objects.objects.clear();               //接受超过64个物体目标后舍弃多余的
			return ;
		}
		
		uint8_t measurementStatus = (can_msg.data[1]&0xE0) >> 5;     //测量状态解析状态
